...
class BaseWriteForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'body': forms.TextInput(attrs={'cols': WRAP_WIDTH, 'rows': 12, 'class': 'mdl-textfield__input', 'id': 'textarea'}),
            'subject': forms.TextInput(attrs={'class': 'mdl-textfield__input', 'id': 'subject'})
        }
...
class WriteForm(BaseWriteForm):
    ...
    recipients = CommaSeparatedUserField(label=(_("Recipients"), _("Recipient")), help_text='', widget=forms.TextInput(attrs={'class': 'mdl-textfield__input', 'id': 'recipient'}))
